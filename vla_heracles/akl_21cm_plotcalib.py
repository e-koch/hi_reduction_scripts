...
# Rui's plot of the bandpass solution
    ...
    merge_eps = 'gs -sDEVICE=pdfwrite -sOutputFile=' + \
    ...
    for i in range(0, num_ant - 1):
        ...
        for j in range(0, len(spw_bpcal)):
            ...
            plotcal()
            subtitle = 'Bandpass Solution: Antenna ' + \
                str(i) + ' SpwID ' + spwid_passcal[j]
            pl.title(subtitle, fontsize=6)

            spw = spwid_passcal[j]
            plotrange = [-1, -1, -45, 45]
            subplot = 2 * 100 + \
                len(spwid_passcal) * 10 + len(spwid_passcal) + j + 1
            xaxis = 'chan'
            yaxis = 'phase'
            figfile = prefix + '.plotcal.bandpass.ant' + str(i) + '.pdf'
            title = 'Passband Solution, Antenna ' + \
                str(i) + 'SpwID ' + spwid_passcal[j]
            plotcal()

        merge_eps = merge_eps + ' ' + prefix + \
            '.plotcal.bandpass.ant' + str(i) + '.pdf'
    ...
    ### Plot phase vs time ###
        ...
        fig_file = plot_dir + field + \
            '_phase_vs_time_' + tag + '.' + plotformat
    ...
    ### Plot amp vs. uv-distance ###
        ...
        fig_file = plot_dir + field + \
            '_amp_vs_uvdist_' + tag + '.' + plotformat
    ...
    ### Plot phase vs. uv-distance ###
        ...
        fig_file = plot_dir + field + \
            '_phase_vs_uvdist_' + tag + '.' + plotformat
...
#-------------------------------------------------------------------------
    if noplotcal != True and noplot == False:
        ...
        for i in range(0, num_ant - 1):
            ...
            title = 'Antenna ' + str(i)
            plotcal()
            subtitle = 'Gain Solution: Antenna ' + \
                str(i) + ' SpwID ' + spwid_passcal[j]
            pl.title(subtitle, fontsize=6)
