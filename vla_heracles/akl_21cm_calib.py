...
def calib_21cm(out_root=None,
    ...
    if source is None or phasecal is None or fluxcal is None or bpcal is None:
    ...
    setjy(vis=vis, field=bpcal, spw=spw_fluxcal)
    ...
    os.system("rm -rf "+out_root+".gaincurve")

    # Create the gaincurve (gaincurve=True in gaincal has been removed)
    gencal(vis=vis, caltable=out_root+".gaincurve", caltype='gc')

    gaincal(vis=vis, field=bpcal, spw=spw_bpcal,
            caltable=out_root+'.intphase_bpcal.gcal', append=False,
            gaintable=out_root+".gaincurve", gaintype="G",
            calmode='p', solint='int', minsnr=2.0, refant=ref_ant)

    os.system("rm -rf "+out_root+".bpcal")
    bandpass(vis=vis, field=bpcal, spw=spw_bpcal,
             selectdata=False,
    ...
    gaincal(vis=vis, field=fluxcal, spw=spw_fluxcal,
            ...
            gaintable=[out_root+'.intphase_bpcal.gcal', out_root+".gaincurve"],
            gainfield=bpcal,
            spwmap=spw_map_bp_to_flux,
            calmode='p', solint='int', minsnr=2.0, minblperant=2,
            refant=ref_ant)
    ...
    gaincal(vis=vis, field=fluxcal, spw=spw_fluxcal,
            ...
            gainfield=bpcal,
            spwmap=spw_map_bp_to_flux,
            calmode='p', solint='inf', minsnr=2.0, minblperant=2,
            refant=ref_ant)
    ...
    fluxscale(vis=vis, caltable=out_root+'.amp.gcal',
              fluxtable=out_root+'.fcal',
              transfer=transfer, reference=fluxcal, refspwmap=ref_spw_map)
