...
def suggest_refant(vis=None, min_sep=50.0, flag_thresh=0.1):
    """
    NAME
    ...
    if vis is None:
        raise NameError("Must specify visibility [vis=]")
    ...
    mean_x = np.mean(ant_pos[0, :])
    mean_y = np.mean(ant_pos[1, :])
    ...
    # sort to get the fraction flagged by antenna
    frac_flagged_by_ant = 0.0 * offset
    for i in range(len(ant_name)):
        flagged = \
            float(flag_statistics['antenna'][ant_name[i].upper()]['flagged'])
    ...
    # indices
    suggest_ind = ((offset > min_sep) &
                  (flagged < (flag_thresh+1.0)*np.mean(flagged))).nonzero()
    ...
    print "Shortest baseline above minimum above flagging threshold: ",\
        ant_name[suggest_ind.min()]

    return ant_name[suggest_ind.min()]
